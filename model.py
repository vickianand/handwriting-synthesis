...
class HandWritingSynthRNN(torch.nn.Module):
    def __init__(
        ...
        # 3 for alpha, beta and kappa and n_gaussians_window for number of
        # components in mixture model for soft-window

        # self.prev_window = torch.zeros(max_stroke_len, batch_size, n_char)
    ...
    def forward(self, inp, c_seq, lstm_in_states=None, prev_window=None, prev_kappa=0):
        ...
        rnn_out = [(torch.stack(first_rnn_out, dim=0), (h, c))]  # [(T, B, memory_cell)]
